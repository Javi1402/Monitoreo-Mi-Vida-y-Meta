...
export async function updateSession(request) {
  ...
  if (request.nextUrl.pathname === "/" && authorizationCode) {
    const isPasswordRecovery = request.cookies.get("mvm_recovery_pending")?.value === "1";
    const callbackUrl = new URL(isPasswordRecovery ? "/auth/recover" : "/auth/confirm", request.url);
